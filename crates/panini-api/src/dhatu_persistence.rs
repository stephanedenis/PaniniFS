...
impl DhatuStore {
    ...
    pub fn open<P: AsRef<Path>>(path: P) -> Result<Self> {
        ...
        let db = DB::open(&opts, path.as_ref())
            .context("Failed to open Dhātu RocksDB")?;
    ...
    pub fn list_profiles(&self) -> Result<Vec<(String, EmotionalProfile)>> {
        ...
        for item in self.db.iterator(IteratorMode::Start) {
            ...
            let profile: EmotionalProfile = serde_json::from_slice(&value)
                .context("Failed to deserialize profile")?;
    ...
    pub fn get_stats(&self) -> Result<StoreStats> {
        ...
        let avg_arousal = profiles.iter()
            .map(|(_, p)| p.intensity.arousal())
            .sum::<f64>() / total as f64;
        
        let avg_confidence = profiles.iter()
        ...
        Ok(StoreStats {
            ...
            avg_arousal,
            avg_confidence,
            emotions: emotions.into_iter().map(|(name, count)| EmotionCount { name, count }).collect(),
        })
    }
